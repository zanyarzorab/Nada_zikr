...
type MainScreen = "home" | "categories" | "reading" | "tasbeeh" | "statistics" | "settings";
...
const QURAN_SURAHS = getSurahs();
const QURAN_TAFSIR_OPTIONS = getTafsirOptions();
...
function QuranScreen({ onBack }: { onBack: () => void }) {
  ...
  const selectedTafsir = QURAN_TAFSIR_OPTIONS.find((option) => option.id === selectedTafsirId);
  const tafsirText = currentAyah
    ? getTafsirForAyah(selectedTafsirId, selectedSurah, currentAyah.ayah) ??
      "Tafsir is not available for this ayah in the selected tafsir yet."
    : "No ayah available.";
  ...
  const handlePreviousAyah = () => {
    if (ayahIndex > 0) setAyahIndex((value) => value - 1);
  };

  const handleNextAyah = () => {
    if (ayahIndex < ayahs.length - 1) setAyahIndex((value) => value + 1);
  };
...
function BottomNav({ current, onNavigate }: { current: MainScreen; onNavigate: (s: MainScreen) => void }) {
  const items: { id: MainScreen; icon: React.ReactNode; label: string }[] = [
    ...
    { id: "statistics", icon: <BarChart2 size={19} />, label: "Stats" },
